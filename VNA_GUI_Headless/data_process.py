...
class DSP:
    ...
    def binary_to_complex(self, in_data):
        val = in_data & 0xFFFF
        real = np.zeros(len(in_data), dtype = np.int32)
        real[in_data >= 32768] = 0xFFFF0000 | in_data[in_data >= 32768]
        real[in_data < 32768] = in_data[in_data < 32768]
        imag = np.zeros(len(in_data), dtype = np.int32)
        imag[:] = in_data >> 16
        return real + 1j * imag
