...
gui_socket.connect(("192.168.0.100", 1234))
...
while True:
    ...
    """
    start = time.time()
    filtered_port1_forward_bytes = io.BytesIO()
    filtered_port2_forward_bytes = io.BytesIO()
    filtered_port1_reverse_bytes = io.BytesIO()
    filtered_port2_reverse_bytes = io.BytesIO()
    np.save(filtered_port1_forward_bytes, filtered_port1_forward)
    np.save(filtered_port2_forward_bytes, filtered_port2_forward)
    np.save(filtered_port1_reverse_bytes, filtered_port1_reverse)
    np.save(filtered_port2_reverse_bytes, filtered_port2_reverse)
    message = {}
    message["filtered_port1_forward"] = filtered_port1_forward_bytes.getvalue()
    message["filtered_port2_forward"] = filtered_port2_forward_bytes.getvalue()
    message["filtered_port1_reverse"] = filtered_port1_reverse_bytes.getvalue()
    message["filtered_port2_reverse"] = filtered_port2_reverse_bytes.getvalue()
    pickled = pickle.dumps(message)
    end = time.time()
    print(f"Saving takes {end - start} s")
    start = time.time()
    websocket.send(pickle.dumps(message))
    end = time.time()
    print(f"Sending takes {end - start} s")
    if first:
        with open("pickled.pickle", "wb") as out:
            out.write(pickled)
        first = False
    """
    start = time.time()
    tempfile = io.BytesIO()
    np.savez(tempfile, filtered_port1_forward = filtered_port1_forward, filtered_port2_forward = filtered_port2_forward, filtered_port1_reverse = filtered_port1_reverse, filtered_port2_reverse = filtered_port2_reverse)
    bytes_to_send = zlib.compress(tempfile.getvalue(), level = 1)
    end = time.time()
    print(f"Saving takes {end - start} s")
    start = time.time()
